...
public interface PersoanaRepository extends JpaRepository<Persoana, Integer> {
    ...
    // Paginare nativa pentru tabelul de persoane
    ...
    // Returnez ultimul ID generat pentru a-l trimite inapoi in frontend dupa INSERT
    @Query(value = "SELECT MAX(id_persoana) FROM Persoane", nativeQuery = true)
    Integer getLastInsertedId();

    // Inserare manuala a datelor unei persoane
    @Modifying
    @Transactional
    ...
    // Cautare dupa inceputul numelui, prenumelui sau CNP-ului
    ...
    // --- RAPOARTE ---
    // Identific persoanele cu cele mai mari datorii neplatite (Rau Platnici)
    ...
    // Identific persoanele care au primit mai multe amenzi decat media (Recidivisti)
    ...
    // Validare unicitate CNP inainte de salvare
